...
internal static class WorkflowTemplates
{
    ...
    /// in the order <see cref="InitializeRunner"/> writes (and reports) them. Discovered from the
    /// embedded resources rather than a hand-kept list, so the <c>Cli/Templates/*.yml</c> glob in
