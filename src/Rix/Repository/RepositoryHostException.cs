...
/// <summary>Thrown by every host here — <see cref="GitHubJobHost"/>, <see cref="GitHubSubmitHost"/>
/// and <see cref="GitHubCiFailureHost"/> — when an operation fails for an operational reason the
...
/// here, so a boundary <c>catch (RepositoryHostException)</c> can't swallow them. Keeps a generic
/// name where the hosts are named for their subcommand, because every one of them throws it and
/// every caller catches it the same way.</summary>
