...
public class InputTests
{
    ...
    => Assert.AreEqual(7, Input.Optional("--timeout", raw, Input.WholeNumber<int>, 7));

    [TestMethod]
    public void Optional_ConstructsFromNonBlank()
    => Assert.AreEqual(5, Input.Optional("--timeout", "5", Input.WholeNumber<int>, 7));

    [TestMethod]
    public void Optional_RejectsMalformed_RatherThanFallingBack()
    {
        var ex = Assert.ThrowsExactly<InvalidInputException>(() => Input.Optional("--timeout", "abc", Input.WholeNumber<int>, 7));
        Assert.AreEqual("--timeout: must be a whole number, got 'abc'", ex.Message);
    }
    ...
    [DataRow("1.5")]
    [DataRow("")]
    public void WholeNumber_RejectsWhatIsNotOne(string raw)
    {
        var ex = Assert.ThrowsExactly<InvalidInputException>(() => Input.WholeNumber<long>(raw));
        Assert.AreEqual($"must be a whole number, got '{raw}'", ex.Message);
    }

    /// <summary>Numbers outside a flag's range parse here and are refused by the type they are
    /// handed to, so this helper has no opinion on them - see <c>TypesTests</c> for the ranges.</summary>
    [TestMethod]
    public void WholeNumber_ParsesAnyWholeNumber_LeavingRangeToTheValueObject()
    {
        Assert.AreEqual(42, Input.WholeNumber<int>("42"));
        Assert.AreEqual(9_000_000_000L, Input.WholeNumber<long>("9000000000"));
        Assert.AreEqual(0, Input.WholeNumber<int>("0"));
        Assert.AreEqual(-5, Input.WholeNumber<int>("-5"));
    }
}
