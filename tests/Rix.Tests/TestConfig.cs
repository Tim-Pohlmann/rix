...
internal static class TestConfig
{
    ...
    => new(new RepoIdentifier(repo), new GitReadToken(readToken), runId);
    ...
    => new
    (
        ValidCiFailure(repo, readToken, runId),
        Valid(repo: repo, prompt: CiFailureJobConfig.PlaceholderPrompt, readToken: readToken, workDir: workDir, outputDir: outputDir)
    );
