...
public class GitHubActionsCiHostTests
{
    ...
    /// reported host failure. It arrives as a <see cref="RepoHostException"/> from the shared
    /// transport and has to be restated in this seam's terms on the way out.</summary>
    [TestMethod]
    public async Task GetRunAsync_WrapsTransportFailure_AndNamesTheOperation()
    {
        ...
        var ex = await Assert.ThrowsExactlyAsync<CiHostException>(
