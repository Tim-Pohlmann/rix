...
public class JobRunnerTests
{
    ...
    public async Task RunAsync_Returns2_WhenClaudeInstallerFails()
    {
        var result = await Startup.ExecuteJobAsync(
            ...
            Context(new StubGit(), FakeRunner(),
    ...
    public async Task RunAsync_RejectsSecondPrForAlreadyQueuedBranch()
    {
        ...
        var result = await JobRunner.RunAsync(MakeConfig(),
            Context(new StubGit(), runner, _ => Task.FromResult<InstallResult>(new Installed())),
            CancellationToken.None);
    ...
    public async Task RunAsync_ConfiguresGitIdentity_InCloneDir()
    {
        ...
        var host = new StubGit(configureIdentity: dir =>
        {
            configuredDir = dir;
        ...
        Assert.IsNotNull(configuredDir, "the clone must be left commit-ready for the agent");
        Assert.AreEqual(agentWorkingDir, configuredDir,
    ...
    {
        var host = new StubGit(
            configureIdentity: _ => throw new RepoHostException("git config failed: exited with code 128"));
    ...
    public async Task RunAsync_CleansUpCloneDir()
    {
        ...
        await JobRunner.RunAsync(MakeConfig(),
            Context(new StubGit(), tracker, _ => Task.FromResult<InstallResult>(new Installed())),
            CancellationToken.None);
    ...
    public async Task RunAsync_Returns1_WhenGitBundleFails()
    {
        ...
        var hostWithFailingBundle = new StubGit(
    ...
    {
        var host = new StubGit(branchExists: _ => Task.FromResult(true));
        RunProcessAsync runner = async (f, a, d, e, onLine, ct) =>
        {
            if (f == "claude")
            {
    ...
    {
        var result = await JobRunner.RunAsync(MakeConfig(),
            Context(new StubGit(), FakeRunner(claudeExitCode: 1), _ => Task.FromResult<InstallResult>(new Installed())),
            CancellationToken.None);
    ...
    {
        var result = await JobRunner.RunAsync(MakeConfig(),
            Context(new StubGit(), FakeRunner(), _ => Task.FromResult<InstallResult>(new InstallFailed("nope"))),
            CancellationToken.None);
