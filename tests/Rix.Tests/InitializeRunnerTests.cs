...
public class InitializeRunnerTests
{
    ...
    private static IReadOnlyList<string> WrittenPaths(IInitializeResult result) => result switch
    {
        InitializeSuccess s => s.WrittenPaths,
    ...
    [TestMethod]
    public async Task RunAsync_WritesBothCallerWorkflows_UnderGithubWorkflows()
    {
    ...
    {
        var result = await Run(TestConfig.ValidInitialize(), Context());
        WrittenPaths(result);
    ...
    public async Task RunAsync_OverwritesSilently_WhenFilesAlreadyExist()
    {
        ...
        WrittenPaths(await Run(config, Context()));
        _writes.Clear();
        var second = await Run(config, Context());
